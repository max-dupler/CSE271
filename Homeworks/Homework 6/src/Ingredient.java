...
public class Ingredient implements Cloneable, Serializable {
    ...
    public static Ingredient parseString(String line)
        ...
        //split string by ; into string array
        String[] ingredients = line.split(";");
        
        //initialize quantity to check for an empty string later
        double quantity = 1.0;
        
        //check for empty string before parsing as a double
        if (ingredients[0].length() > 0) {
            try {
                quantity = Double.parseDouble(ingredients[0]);
            } catch (NumberFormatException e) {
                throw new InvalidIngredientException(EXP_MSG);
            }
        }
        
        //return a new ingredient with the parameters from String line
        return new Ingredient(quantity, ingredients[1].trim(), 
                ingredients[2].trim()); 
    }
    ...
    public boolean equals(Object other) {
        ...
        //check if other object is an ingredient
        if (other instanceof Ingredient) {
            
            //cast other as ingredient
            Ingredient check = (Ingredient) other;
            
            /* return true if the measurement, quantity 
             * and description are the same
             */
            return this.getDescription().equals(check.getDescription())
                    && this.getMeasurement().equals(check.getMeasurement())
                    && this.getQuantity() == check.getQuantity();
        }
        
        //return false if other isn't an ingredient
        return false;
    }

    /**.
     * creates a clone of an ingredient
     * @return the cloned ingredient
     */
    @Override
    public Ingredient clone() {
        return new Ingredient(this.getQuantity(), 
                this.getMeasurement(), this.getDescription());
    }
}
